...
# # # Configuration
# # # -------------------------------------------------------------------------
# # DATA_PATH = r"C:\Users\INDIA\Desktop\sihPS2\bokaro_to_cmo_customers.csv"
# # MODEL_DIR = r"C:\Users\INDIA\Desktop\sihPS2\saved_models"
# # OUTPUT_PATH = r"C:\Users\INDIA\Desktop\sihPS2\optimized_rake_allocations.csv"
...
# #         # Enhanced multi-objective cost: cost - reliability_reward - priority_reward
...
# #         # Reward reliability and priority
# #         on_time_prob = float(row.get("on_time_prob", 0))
# #         reliability_reward = RELIABILITY_WEIGHT * on_time_prob * y[i]    # encourages choosing reliable (on-time) rail routes
# #         priority_reward = PRIORITY_WEIGHT * prio * y[i]     # keeps your priority weight

# #         # Combine terms — note negative signs for rewards (we’re minimizing)
# #         objective.append(rail_cost_term + road_cost_term + fixed_costs - reliability_reward - priority_reward)
...
# #         feasible = 1 if (rake_avail == 1 and on_time == 1) else 0

# #         if feasible == 0:
# #             prob += y[i] == 0, f"Feasible_{i}"

# #         prob += q[i] <= planned_qty * y[i], f"Qty_Upper_{i}"
# #         if min_rake_tonnage > 0:
...
# #             if slots > 0:
# #                 prob += lpSum([y[i] for i in grp.index]) <= slots, f"SidingSlots_{yard}"

# #     if {"yard_id", "inventory_t", "production_forecast_t"} <= set(df.columns):
# #         for yard, grp in df.groterm_upby("yard_id"):
# #             cap = float((grp["inventory_t"].fillna(0) + grp["production_forecast_t"].fillna(0)).iloc[0])
# #             prob += lpSum([q[i] for i in grp.index]) <= cap, f"Inventory_{yard}"

# #     print("Solving MILP...")
# #     prob.solve(PULP_CBC_CMD(msg=False))
# #     print(f"Solver Status: {LpStatus[prob.status]}")

# #     df["y_rail"] = [int(y[i].value()) for i in df.index]
# #     df["q_rail_tons"] = [float(q[i].value()) for i in df.index]
...
# #         + row.get("pred_road_cost_total", 0) * (1 - df.loc[i, "q_rail_tons"] / max(row.get("planned_qty_t", 1), 1))
# #         for i, row in df.iterrows()
# #     ]
...
# #     print(f"✅ Optimization completed and saved to {OUTPUT_PATH}")
# #     return df
...
# #     print(result_df[["plan_id", "optimized_mode", "q_rail_tons", "optimized_total_cost"]].head())
...
# # ON_TIME_THRESHOLD = 0.6
# # PRIORITY_WEIGHT = 0.05
# # RELIABILITY_WEIGHT = 0.03  # Added missing constant
...
# #     if {"yard_id", "inventory_t", "production_forecast_t"} <= set(df.columns):
# #         for yard, grp in df.groupby("yard_id"):  # Fixed typo: groterm_upby -> groupby
# #             cap = float((grp["inventory_t"].fillna(0) + grp["production_forecast_t"].fillna(0)).iloc[0])
# #             prob += lpSum([q[i] for i in grp.index]) <= cap, f"Inventory_{yard}"
...
# ON_TIME_THRESHOLD = 0.6
# PRIORITY_WEIGHT = 0.05
# RELIABILITY_WEIGHT = 0.10  # Increased to make reliability more valuable
# MIN_RAKE_PROPORTION = 0.3  # At least 30% of feasible orders should use rail
...
#         feasible = 1 if (rake_avail == 1 and on_time_label == 1) else 0
...
#         # Stronger rewards for reliability and priority
#         on_time_prob = float(row.get("on_time_prob", 0))
...
#         sustainability_bonus = 0.02 * planned_qty * y[i]  # Additional incentive for rail
...
#     # Add minimum rail utilization constraint
#     if feasible_rail_indices:
#         min_rail_orders = max(1, int(MIN_RAKE_PROPORTION * len(feasible_rail_indices)))
#         prob += lpSum([y[i] for i in feasible_rail_indices]) >= min_rail_orders, "Min_Rail_Utilization"
...
#         alt_output_path = f"C:\\Users\\INDIA\\Desktop\\sihPS2\\optimized_rake_allocations_{timestamp}.csv"
#         df.to_csv(alt_output_path, index=False)
#         print(f"⚠️ Original file was locked. Saved to: {alt_output_path}")
#         OUTPUT_PATH = alt_output_path
#     except Exception as e:
#         print(f"❌ Error saving file: {e}")
#         print("Displaying results instead...")
#         print(df[["plan_id", "optimized_mode", "q_rail_tons", "optimized_total_cost"]].head())
